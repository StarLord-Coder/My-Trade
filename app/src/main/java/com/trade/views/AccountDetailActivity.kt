...
//                rlLoading?.visibility = View.GONE
    ...
    private fun confirmFilter() {
        if (!tvEndDate?.text.toString().isNullOrEmpty() && !tvStartDate?.text.toString().isNullOrEmpty()) {
            ...
            imgBox1?.setImageDrawable(resources.getDrawable(R.drawable.img_account_detail_box_1_red_app_2, null))
        } else {
            startDate = ""
            endDate = ""
            getTransactionData()
            imgBox1?.setImageDrawable(resources.getDrawable(R.drawable.img_account_detail_box_1_app_2, null))
            filterView?.visibility = View.GONE
        }
    }
